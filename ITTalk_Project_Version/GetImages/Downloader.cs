    ...
    public static class Downloader
    {
        ...
        private static Scene _GenerateScene(double ratio)
        {
            ...
            lights.Add(new Light() { 
                ...
                Color = RayTraceCore.Color.Make(.49,.07,.07) });
            lights.Add(new Light() { 
                Pos = Vector.Make(1.5,2.5,1.5),
                Color = RayTraceCore.Color.Make(.07,.07,.49) });
            lights.Add(new Light() { 
                Pos = Vector.Make(1.5,2.5,-1.5),
                Color = RayTraceCore.Color.Make(.07,.49,.071) });
            lights.Add(new Light() { 
                Pos = Vector.Make(0,3.5,0),
                Color = RayTraceCore.Color.Make(.21,.21,.35) });
